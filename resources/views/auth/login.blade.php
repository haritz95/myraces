<x-guest-layout>
    <div class="px-6 pt-8 pb-7">
        <h2 class="text-xl font-extrabold text-slate-900 mb-1">Bienvenido de nuevo</h2>
        <p class="text-sm text-slate-500 mb-7">Inicia sesión en tu cuenta</p>
        ...
        <div class="space-y-2.5 mb-6">
            <a href="{{ route('social.redirect', 'google') }}"
               class="flex items-center justify-center gap-3 w-full border border-slate-200 rounded-full px-4 py-3 text-sm font-semibold text-slate-700 hover:bg-slate-50 hover:border-slate-300 transition-colors">
        ...
        <form method="POST" action="{{ route('login') }}" class="space-y-4">
            ...
            <div class="flex items-center justify-between">
                ...
                @if (Route::has('password.request'))
                    <a href="{{ route('password.request') }}" class="text-sm text-primary hover:text-orange-600 font-semibold transition-colors">
        ...
        <p class="text-center text-sm text-slate-500 mt-6">
            ¿No tienes cuenta?
            <a href="{{ route('register') }}" class="text-primary font-bold hover:text-orange-600 transition-colors">Regístrate</a>
        </p>
    </div>
</x-guest-layout>
