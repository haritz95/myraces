<x-guest-layout>
    ...
    <div class="relative overflow-hidden rounded-2xl mb-8 mt-2 py-10 px-6"
        ...
        <div class="relative">
            <h1 class="font-black italic tracking-tighter uppercase leading-[0.88] text-white"
                ...
                {{ __('auth.welcome_back') }}<span style="color:rgb(var(--color-primary))">.</span>
            </h1>
    ...
    <form method="POST" action="{{ route('login') }}" class="space-y-6">
        ...
        <div>
            <div class="flex justify-between items-end mb-2">
                <label for="password" class="block text-[10px] font-bold uppercase tracking-widest"
                ...
                @if (Route::has('password.request'))
                    <a href="{{ route('password.request') }}"
                       ...
                       style="color:rgb(var(--color-primary) / 0.65)" onmouseenter="this.style.color='rgb(var(--color-primary))'" onmouseleave="this.style.color='rgb(var(--color-primary) / 0.65)'">
        ...
        <label class="flex items-center gap-3 cursor-pointer select-none">
            ...
            <span class="text-[11px] font-bold uppercase tracking-widest" style="color:rgba(255,255,255,0.30)">{{ __('auth.remember_me') }}</span>
        </label>
    ...
    <p class="text-center text-sm mt-8 mb-2" style="color:rgba(255,255,255,0.30)">
        {{ __('auth.no_account') }}
        <a href="{{ route('register') }}" class="font-bold transition-colors ml-1"
